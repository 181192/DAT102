...
public class KjedetOrdnetListe<T extends Comparable<T>> implements OrdnetListeADT<T> {
	...
	public int lengde() {
		int lengde = lengdeRek(foerste);
		return lengde;
	}

	private int lengdeRek(LinearNode<T> aktuell) {
		int lengde;
		if (aktuell == null) {
			lengde = 0;
		} else {
	...
	private String toStringRek2(LinearNode<T> aktuell) {
		...
		if (aktuell == null) {
			...
			resultat = toStringRek2(aktuell.getNeste()) + (aktuell.getElement()).toString() + "\t";
		}
		return resultat;
	}
}
